...
/**
 * An error handler middleware: returns a handle function, which can
 ...
 * @memberof errors
 * @return {Function} Handle function which can be called with an error.
 */
const handler = (req, res) => {
  return (error) => {
    ...
    const stack = error ? error.stack : 'Internal Error';
    const status = error ? error.status : 500

    logging.error(error, { email, stack: stack })
    res.status(status).end('Internal server error.')
  }
}
